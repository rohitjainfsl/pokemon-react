      ...
      <div className="w-[40rem] mx-auto py-4 text-center">
        ...
        <div className="flex items-center justify-between gap-4 px-4">
          <select
            name=""
            id=""
            className="w-1/3 bg-white border border-slate-300 rounded-md py-2 px-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm"
          >
            <option value="" selected disabled>
              Filter By Type
            </option>
            {allTypes.map((typeObject, index) => (
              <option key={index}>{typeObject.name}</option>
            ))}
          </select>
